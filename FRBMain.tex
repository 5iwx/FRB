...
【姜、崔】
\subsubsection{位置隐私保护常用方法}
- 模糊、虚拟位置：对自身位置进行泛化，给服务提供商一个模糊的范围。
  - 分布式模糊：和多个邻居设备进行协同，协同共同生成一个模糊范围。
  - 差分隐私模糊：选取几个随机扰动点，拟定一个模糊范围。但对用户设备算力消耗大。
  - K-匿名：选取多个邻居设备，生成一个范围，范围内节点皆匿名，服务商无法区分哪个是当前用户。这个方法可以和分布式架构很好结合。但不能提供对服务提供商的权益保护，即不能防止用户造假。
- 假身份：使用假ID和服务提供商进行通讯，是一种匿名手段，但可能被服务提供商通过行为习惯分析等手段去匿名化，需要经常性更换ID保证隐私效果。
- 加密方法：同态加密等技术，让服务提供商不知道具体数据的情况下进行服务提供。但同态加密等密码算法开销非常大。
